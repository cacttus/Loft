...
namespace PirateCraft
{
  ...
  public class MtFontPatchInfo
  {
    ...
    //** Char Info is not needed after generating CachedChars.
    public StbTrueTypeSharp.StbTrueType.stbtt_packedchar[] CharInfo = null;
    //packed Charinfo is particular to an image size so belongs on here.
    ...
    //public Dictionary<LanguageCode, List<CharacterRangeUTF8>> LangRanges = new Dictionary<LanguageCode, List<CharacterRangeUTF8>>();

    // public bool HasChar(LanguageCode c, int unicode_point)
    // {
    //   //This may actually be faster..
    //   return CachedChars.Keys.Contains(unicode_point);

    //   // if (LangRanges.TryGetValue(c, out var ranges))
    //   // {
    //   //   foreach (var range in ranges)
    //   //   {
    //   //     if (unicode_point < range.FirstChar || unicode_point >= (range.FirstChar + range.CharCount))
    //   //     {
    //   //       return true;
    //   //     }
    //   //   }
    ...
    public bool GetChar(int unicode_point, float fontSize, out MtCachedCharData ccd, out float scale)
    {
      ...
      {
        unicode_point = ' ';//default to space if we don't contain it
        return false;
      }
    }
  ...
  //   public SortedDictionary<int, FontPatchRangeUTF8> CharacterRanges { get; private set; } = new SortedDictionary<int, FontPatchRangeUTF8>();//Int maps to the First UTF-8 Char
  //   public LanguagePackUTF8(LanguageCode c, List<FontPatchRangeUTF8> ranges)
  //   {
  //     LanguageCode = c;
  ...
  //   }
  // }
  // public class PackedLanguagesUTF8
  // {
  //   Dictionary<FontPatchRangeUTF8 CharacterRanges
  //   //LanguageCode -> <Character ID> -> PackedLanguagesUTF8
  //   //char ranges for multiple languages.
  // }
  //   public class LanguagePacks
  //   {
  //     private static Dictionary<LanguageCode, UnicodeRange> Packs = new Dictionary<LanguageCode, UnicodeRange>();

  //     public static void Load()
  //     {
  // GetUnicodeRangeForLanguageCode

  //       Packs = Packs.ConstructIfNeeded();
  //       Packs.Add(LanguageCode.en, new List<UnicodeRange>() { new FontPatchRangeUTF8(UnicodeRanges.BasicLatin});
  //       Packs.Add(LanguageCode.es, new List<FontPatchRangeUTF8>() { new FontPatchRangeUTF8(' ', 512) });//Same as EN

  //       if (Gu.EngineConfig.UseLang_RU)
  //       {
  //         addPack(LanguageCode.ru, new List<FontPatchRangeUTF8>() { new FontPatchRangeUTF8(0x400, 0xFE2F - 0x400) });
  //       }
  //       if (Gu.EngineConfig.UseLang_ZH)
  //       {
  //         addPack(LanguageCode.zh, new List<FontPatchRangeUTF8>() { new FontPatchRangeUTF8(0x4E00, 0x62FF - 0x4E00) });//0x9FFF is the whole range, that's a lot
  //       }
  //     }
  //   }
  ...
  public class MtFontLoader : MtLoader
  {
    ...
    private List<LanguageCode> Languages = null;
    private List<FontRangeUTF8> SupportedFontRanges = null;

    //This should be the final structure
    public Dictionary<MtCachedCharData, Dictionary<float, MtFontPatchInfo>> CachedChars = new Dictionary<MtCachedCharData, Dictionary<float, MtFontPatchInfo>>();
    ...
    private const float c_fSupportGlyphPercent = 0.5f;//pct of glyphs that we say this is supported.
    ...
    private void GetSupportedRanges()
    {
      //honestly this si something..we need to cache..
      
      //Checks to see if this font can support a given codepage
      //Now, i'm not sure if this is correct, exactly. But it doesn't appear that Fonts (at least, STB) contain any region/language info.
      //Probably wrong on this, but for now.

      //Get all languages we want to support

      //We can only check for support for *unicode page ranges* this is how unicode works
      //so if the language changes when game loads..
      Languages = Languages.ConstructIfNeeded();
      Languages.Add(GameTextCulture.GetDefaultLanguage());
      if (Gu.EngineConfig.UseLang_RU)
      {
        //So this is just testing out multiple languages..
      ...
      //Get all ranges for all langauges we want to support
      var all_ranges = new List<FontRangeUTF8>();
      foreach (var lc in Languages)
      {
        var ranges = GameTextCulture.GetUnicodeRangesForLanguageCodeSorted(lc);
        all_ranges.AddRange(ranges);
      }

      //TODO:  Eliminate duplicate ranges. 

      //Check if supported, 
      SupportedFontRanges = SupportedFontRanges.ConstructIfNeeded();
      unsafe
      {
        foreach (var range in all_ranges)
        {
          int nSupported = 0;
          for (int ci = range.FirstChar; ci < range.FirstChar + range.CharCount; ci++)
          {
            int supported = StbTrueTypeSharp.StbTrueType.stbtt_FindGlyphIndex(_fontInfo, ci);

            if (supported != 0) { nSupported++; }
            
            //we can easily exit this func if we're over a threshold.
            // int charsleft = range.CharCount - ci;
            // var supp = (float)nSupported / (float)range.CharCount;//..
          }

          if ((float)nSupported / (float)range.CharCount > c_fSupportGlyphPercent)
          {
            SupportedFontRanges.Add(range);
          }
    ...
    private void CreateFontImages()
    {
      ...
      for (int iTex = 0; Gu.WhileTrueGuard(iTex, Gu.c_intMaxWhileTrueLoopSmall); iTex++)
      {
        ...
        var ranges = new StbTrueTypeSharp.StbTrueType.stbtt_pack_range[2];
        unsafe
        {

          ranges[0] = new StbTrueTypeSharp.StbTrueType.stbtt_pack_range();
          ranges[0].font_size = (float)charsize;
          ranges[0].first_unicode_codepoint_in_range = ' ';
          ranges[0].num_chars = 511;
          ranges[0].array_of_unicode_codepoints = null;
          ranges[0].chardata_for_range = (StbTrueTypeSharp.StbTrueType.stbtt_packedchar*)Marshal.AllocHGlobal(sizeof(StbTrueTypeSharp.StbTrueType.stbtt_packedchar) * 511).ToPointer();
          //ranges[0].chardata_for_range = 
          // ranges[0].chardata_for_range = ;
          ranges[1] = new StbTrueTypeSharp.StbTrueType.stbtt_pack_range();
          ranges[1].font_size = (float)charsize;
          ranges[1].first_unicode_codepoint_in_range = 'я';
          ranges[1].num_chars = 1;
          ranges[1].array_of_unicode_codepoints = null;
          ranges[1].chardata_for_range = (StbTrueTypeSharp.StbTrueType.stbtt_packedchar*)Marshal.AllocHGlobal(sizeof(StbTrueTypeSharp.StbTrueType.stbtt_packedchar) * 1).ToPointer();
          // ranges[1].chardata_for_range = ;      
        }


        byte[] atlasData = ConstructBitmapForSize(imageWidth, imageHeight, charPadding, ranges);
        ...
        int usedHeight = GetBitmapUsedHeight(ranges, imageWidth, imageHeight);
        atlasData = TrimAtlasBitmapToUsedHeight(imageWidth, usedHeight, atlasData);

        if (atlasData != null)
        {
          //Set the megatex image.
          Img32 img = CreateFontImage(atlasData, imageWidth, usedHeight);
    ...
    private int GetBitmapUsedHeight(StbTrueTypeSharp.StbTrueType.stbtt_pack_range[] ranges, int image_Width, int image_height)
    {
      int maxHeight = 1;
      ...
      foreach (var range in ranges)
      {
        unsafe
        {
          ...
          for (int cCode = range.first_unicode_codepoint_in_range; cCode < range.num_chars; cCode++)
          {
            StbTrueTypeSharp.StbTrueType.stbtt_GetPackedQuad(range.chardata_for_range, image_Width, image_height, cCode, &curX, &curY, &stbQuad, 0);

            quadMinY = stbQuad.t0 * image_height;
            quadMaxY = stbQuad.t1 * image_height;
    ...
    private Img32 CreateFontImage(byte[] pData, int image_width, int image_height)
    {
    ...
    private byte[] ConstructBitmapForSize(int img_width, int img_height, int img_padding, StbTrueTypeSharp.StbTrueType.stbtt_pack_range[] ranges)
    {
      Gu.Assert(ranges != null);
      ...
      unsafe
      {
        ...
        fixed (byte* atlasData_pinned = atlasData)
        {
          ...
          else
          {
            ...
            {
              fixed (StbTrueTypeSharp.StbTrueType.stbtt_pack_range* ranges_pinned = ranges)
              {
                ret = StbTrueTypeSharp.StbTrueType.stbtt_PackFontRanges(context, _pFontBuffer_pinned, 0, ranges_pinned, ranges.Length);
              }
            }
            if (ret == 0)
            {
              Gu.Log.Error("Failed to pack font range size=" + ranges[0].font_size);
              Gu.DebugBreak();
              atlasData = null;
            }
  ...
  public class MegaTex
  {
    ...
    public MtFontLoader GetFont(LanguageCode lc, FontFace fontLoc, UiFontStyle style)
    {
      Gu.Assert(fontLoc != null);
      var loc = fontLoc.GetFontFileLoc(lc, style);
      if (_locToFileCache.TryGetValue(loc, out var ret))
      {
